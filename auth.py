...
def login() -> Optional[RedirectResponse]:
    ...
    with ui.row().style('position: relative; width: 100vw; height: 100vh; row-gap: 0px; column-gap: 0px;'):
        ...
        for _ in range(9):  # Adjust the range based on how many tiles you need
            ui.image("./image2.jpg").style('width: 32vw; height: 33vh; object-fit: contain;opacity: 0.7;')
        with ui.card().classes('absolute-center').style('width: 300px; padding: 20px; background-color: rgba(255, 255, 255, 0.9); border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); float: right;'):
